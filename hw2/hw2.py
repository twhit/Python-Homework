...
# Determine the operation based on the presence of and/or keywords
if ("or" in splitquery and "and" not in splitquery):
    operation = "OR"
else:
    operation = "AND"

# Create a set to hold the query search words
s = set()
for item in splitquery:
    if (item != "and" and item != "or"):
        s.add(item)

print("Performing " + operation + " search for: " + str(s) + "\n") 

for i,quote in enumerate(data_list):
    words = quote.split()
    # Create a set to hold the words in a quote
    t = set()
    for word in words:
        t.add(word)
    if (operation == "AND"):
    # If the intersection of the two sets is equal to s, then
    # all search words are in the quote
        if (s&t == s):
            print("Found at", i, quote + "\n")
    elif (operation == "OR"):
    # If the size of the intersection of the two sets is > 0,
    # at least one of the search words is in the quote
        if (len(s&t) != 0):	
            print("Found at", i, quote + "\n")
